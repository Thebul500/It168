...
public class TextMenu
{
	...
	/**
	 * @param menuItems An array of Strings that represent the various menu
	 *                  labels. The array must be the exact size to hold the
	 *                  number of menu items desired.
	 */
	...
	public int getChoice(Scanner scan)
	{
		...
		this.displayMenu();
		System.out.print(
				"Please enter your choice(1-" + menuItems.length + "): ");
		input = scan.next();
		...
		while (choice == -1)
		{
			...
			System.out.print(
					"Enter a number between 1 and " + menuItems.length + ": ");
			input = scan.next();
			choice = this.validateChoice(input);
		}
		return choice;
